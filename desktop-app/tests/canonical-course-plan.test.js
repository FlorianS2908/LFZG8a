...
const { normalizeCanonicalPlan, validateCanonicalPlan, enrichPlanWithContainerConfiguration, toClassbookModel } = require('../app/lib/content-factory/course-planning/canonical-course-plan');
...
test('kanonischer Unterrichtsplan validiert feste und variable UE-Verteilungen streng', () => {
  ...
  assert.match(validateCanonicalPlan(incomplete, { totalDays: 2, totalUnits: 4, unitsByDay: [2, 2] }).errors.join(' '), /Inhalt fehlt.*Kompetenzziel fehlt.*Arbeitsform/s);
});
