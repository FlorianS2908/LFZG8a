function renderTestProtocolHtml(protocol = {}) {
  ...
  <style>
    ...
    <section class="card"><h2>KI-Konfiguration & Kosten</h2><p>AI_PROVIDER: ${escapeHtml(protocol.aiConfig?.aiProvider || 'local')} | OpenAI configured: ${protocol.aiConfig?.openAiConfigured ? 'ja' : 'nein'} | Modell: ${escapeHtml(protocol.aiConfig?.openAiModel || '-')} | keySource: ${escapeHtml(protocol.aiConfig?.keySource || 'missing')}</p><p>Geschaetzte Kosten: ${escapeHtml(protocol.costEstimate?.estimatedCostUsd ?? 0)} USD</p></section>
    <section class="card"><h2>Prompt Contracts & KI-Laeufe</h2>${table(['Purpose','Provider','Prompt Contract','Prompt Score','Output Review','Fallback','Repair'], (protocol.aiRuns || []).map((run) => [run.purpose, run.provider, run.promptContract || `${run.promptId} ${run.promptVersion}`, run.promptScore ?? run.promptQualityScore, `${run.outputReviewStatus} (${run.outputReviewScore})`, run.fallbackUsed ? 'ja' : 'nein', run.repairUsed ? 'ja' : 'nein']))}</section>
