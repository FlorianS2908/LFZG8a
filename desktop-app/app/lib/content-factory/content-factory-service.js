...
function createContentFactoryService({ appData, projectRoot = process.cwd(), safeStorage, migrationPath, coursePlanningAiOrchestrator = null }) {
  ...
  return {
    ...
    decideCoursePlanConflict: (projectId, version, input, session) => { assertAdmin(session); ensureFactory(); return coursePlanning.decidePlanConflict(projectId, version, input); },
    editCoursePlanUnit: (projectId, version, input, session) => { assertAdmin(session); ensureFactory(); return coursePlanning.editPlanUnit(projectId, version, input); },
    confirmCoursePlanReview: (projectId, version, session) => { assertAdmin(session); ensureFactory(); return coursePlanning.confirmReviewedPlan(projectId, version); },
    acceptCoursePlanReview: (projectId, version, session) => { assertAdmin(session); ensureFactory(); return coursePlanning.acceptReviewedPlan(projectId, version); },
    applyCoursePlanConfiguration: (projectId, profile, session) => { assertAdmin(session); ensureFactory(); return coursePlanning.applyContainerConfiguration(projectId, profile); },
    getCoursePlanClassbookModel: (projectId, version, session) => { assertAdmin(session); ensureFactory(); return coursePlanning.getClassbookModel(projectId, version); },
    exportCoursePlanXlsx: (projectId, version, outputPath, session, options) => { assertAdmin(session); ensureFactory(); return coursePlanning.exportStoredCoursePlan(projectId, version, outputPath, options); },
    ...
    updateTopicReview: (projectId, input, session) => { assertAdmin(session); ensureFactory(); return coursePlanning.updateTopicReview(projectId, input); },
    confirmTopicReview: (projectId, session) => { assertAdmin(session); ensureFactory(); return coursePlanning.confirmTopicReview(projectId, session?.userId || null); },
